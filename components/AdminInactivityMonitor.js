...
export default function AdminInactivityMonitor({ userId }) {
  ...
  const checkSessionExpiry = async () => {
    ...
    if (lastActivity) {
      ...
      if (timeSinceActivity > ADMIN_SESSION_MAX_AGE) {
        console.log(
          "[ADMIN INACTIVITY] Session expired - last activity was",
  ...
  const resetTimer = () => {
    ...
    console.log(
      "[ADMIN INACTIVITY] Timer reset - will logout after 1 hour of inactivity",
    );
  };
  ...
  useEffect(() => {
    ...
    console.log(
      "[ADMIN INACTIVITY] Checking admin session validity for user:",
      userId,
    );
    ...
    checkSessionExpiry().then((isValid) => {
      if (!isValid) {
        console.log(
          "[ADMIN INACTIVITY] Admin session invalid - redirecting to signin",
        );
        return;
      }
