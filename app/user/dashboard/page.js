...
export default function UserDashboard() {
  ...
  const fetchCryptoData = async () => {
    try {
      ...
      const totalMarketCap = transformedData.reduce(
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-white/90">
                ...
                <span className="text-lg font-semibold">
                  {cryptoLoading
                    ? "Loading..."
                    : `${topCryptoAssets.length} assets tracked`}
                </span>
              </div>
            </div>
              ...
              <>
                <div>
                  ...
                  <p className="text-xl font-bold">
                    {profile.currency}{" "}
                    {topCryptoAssets.find((c) => c.symbol === "BTC")
                      ? formatPrice(
                          topCryptoAssets.find((c) => c.symbol === "BTC").value,
                        )
                      : "N/A"}
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-xl font-bold">
                    {profile.currency}{" "}
                    {topCryptoAssets.find((c) => c.symbol === "ETH")
                      ? formatPrice(
                          topCryptoAssets.find((c) => c.symbol === "ETH").value,
                        )
                      : "N/A"}
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-xl font-bold">
                    {profile.currency}{" "}
                    {formatPrice(
                      cryptoData.reduce((sum, c) => sum + c.marketCap, 0) / 1e9,
                      1,
                    )}
                    B
                  </p>
...
function ActionCard({ icon, title, color, onClick, isDarkMode }) {
  ...
  const colorClasses = isDarkMode
    ? {
        ...
        purple:
          "bg-gradient-to-br from-purple-500/20 to-fuchsia-500/20 border-purple-500/30 hover:border-purple-500/50",
      }
    : {
        emerald:
          "bg-white border-indigo-200 hover:border-indigo-300 shadow-sm hover:shadow-md hover:shadow-indigo-200/50",
...
function CryptoCard({ asset, currency, formatPrice, isDarkMode }) {
  const sparklineData =
    asset.sparkline && asset.sparkline.length > 0
      ...
      : Array.from(
          { length: 24 },
          (_, i) => asset.value * (1 + Math.random() * 0.02 - 0.01),
        );
  ...
  return (
    ...
    >
      ...
      <div className="mb-2">
        ...
        >
          {currency} {formatPrice(asset.value)}
        </p>
        <p
        ...
        >
          <defs>
            ...
            >
              <stop
                offset="0%"
                stopColor={asset.trend === "up" ? "#10b981" : "#ef4444"}
                stopOpacity="0.4"
              />
              <stop
                offset="100%"
